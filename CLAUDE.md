...
**Bloqueo agresivo:** `LauncherRoot` tiene un tick por minuto (`nowTick`) y calcula `blockedPackages` = las apps distractoras si hay una sesión activa (`isActiveAt`). Ese set se pasa a `HomeScreen` y `AppDrawer`; `AppRow` con `blocked = true` se **grisa y deshabilita el tap de lanzamiento** (el long-press sigue funcionando para gestionarla). El auto-launch de búsqueda ignora apps bloqueadas, y el acceso rápido se rutea por `onAppClick` para respetar el bloqueo. `onAppClick` es el único choke point: orden límite diario → sesión de foco (firme, `FocusBlockDialog`) → fricción → lanzar. Todo es solo-desde-Foco (sin servicio en background). La UI de Modo Foco vive en una única sección de Ajustes (sesiones + fricción + apps distractoras + escala de grises).

**Escala de grises:** `util/Grayscale.kt` togglea el color-correction del sistema (daltonizer en monochromacy) vía `Settings.Secure`, lo cual requiere `WRITE_SECURE_SETTINGS` — no solicitable por dialog; el usuario lo concede una vez por adb (`pm grant … WRITE_SECURE_SETTINGS`). Con `grayscaleInFocus` activo, un `LaunchedEffect` en `LauncherRoot` pone el sistema en grises cuando hay sesión activa y lo saca al terminar (best-effort mientras Foco esté vivo). Al desactivar el toggle se restaura el color. **Fallback sin adb:** si falta el permiso, Ajustes muestra el comando adb (seleccionable) *y* una opción "Activar manualmente" que abre Accesibilidad → Corrección de color; durante una sesión activa, si no se puede togglear solo, el inicio muestra un nudge tappable (`home_grayscale_nudge`) que abre esos ajustes.
